    ...
    public function toMail($notifiable)
    {
        if ($this->user->isAdmin()) {
            return (new MailMessage)
                        ->subject('Una Orden ha sido Aprobada')
                        ->line('La orden #'.$this->order->id. ' con número de Orden de Producción #'.$this->order->consecutivo_op.' ha sido **aprobada**. Para revisar más detalles de la misma, haz click en el siguiente botón:')
            ...
            return (new MailMessage)
                        ->subject('Tu Orden ha sido Aprobada')
                        ->line('Tu orden con ID #'.$this->order->id.' ha sido **aprobada**. Detalles rápidos de la orden:')
