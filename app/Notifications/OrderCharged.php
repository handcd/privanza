...
class OrderCharged extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        if ($this->user->isAdmin()) {
            return (new MailMessage)
                        ->subject('Una Orden ha sido Cobrada')
                        ->line('La orden #'.$this->order->id. ' con número de Orden de Producción #'.$this->order->consecutivo_op.' ha sido **cobrada**. Para revisar más detalles de la misma, haz click en el siguiente botón:')
            ...
            return (new MailMessage)
                        ->subject('Tu Orden ha sido Cobrada')
                        ->line('Tu orden con ID #'.$this->order->id.' ha sido **cobrada**. Detalles rápidos de la orden:')
